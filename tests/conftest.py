...
from mytestsdk3 import Mytestsdk3, AsyncMytestsdk3, DefaultAioHttpClient
from mytestsdk3._utils import is_dict
...
logging.getLogger("mytestsdk3").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Mytestsdk3]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Mytestsdk3(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncMytestsdk3]:
